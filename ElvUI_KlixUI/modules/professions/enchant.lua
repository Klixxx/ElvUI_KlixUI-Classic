...
function Pr:EnchantButton()
	local button = T.CreateFrame("Button", "KUI_EnchScrollButton", _G["TradeSkillFrame"], "MagicButtonTemplate")
	...
	button:SetScript("OnClick", function()
		-- if (IsShiftKeyDown() and button.itemID) then
			-- local activeEditBox = T.ChatEdit_GetActiveWindow()
			-- if activeEditBox then
				-- local _, link = T.GetItemInfo(button.itemID)
				-- T.ChatEdit_InsertLink(link)
			-- end
		-- else
			T.CraftRecipe(_G["TradeSkillFrame"].DetailsFrame.selectedRecipeID)
			T.UseItemByName(38682)
		-- end
	end)
	...
	local function UpdateScrollButton(frame)
		...
		local CURRENT_TRADESKILL = T.select(7, T.GetTradeSkillLine())
		if CURRENT_TRADESKILL ~= EnchName then _G["KUI_EnchScrollButton"]:Hide() return end

		local recipeInfo = T.GetRecipeInfo(frame.selectedRecipeID)

		if not recipeInfo then return end
		if T.IsTradeSkillGuild() or T.IsTradeSkillLinked() then
			_G["KUI_EnchScrollButton"]:Hide()
		elseif recipeInfo.alternateVerb then
			-- button.itemID = Pr.mapSpellToItem[recipeInfo.recipeID]
			-- if (not button.itemID) then
				-- KUI:Print(string.format(L["Missing scroll item for spellID %d. Please report this at the KlixUI Ticket Tracker."], recipeInfo.recipeID))
			-- end
			_G["KUI_EnchScrollButton"]:Show()
			local scrollnum = T.GetItemCount(38682)
			_G["KUI_EnchScrollButton"]:SetText(T.string_format("%s (%d)", L["Scroll"], scrollnum))
			if recipeInfo.craftable and recipeInfo.learned and scrollnum > 0 then
				_G["KUI_EnchScrollButton"]:Enable()
			else
				_G["KUI_EnchScrollButton"]:Disable()
			end
		else
			_G["KUI_EnchScrollButton"]:Hide()
		end
