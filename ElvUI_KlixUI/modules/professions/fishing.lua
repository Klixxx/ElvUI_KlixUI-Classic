...
local FL = LibStub("LibFishing-1.0-KUI")

local SavedWFOnMouseDown

function Pr:HijackFishingCheck()
	if ( not Pr.AddingLure and not T.InCombatLockdown() and (not T.IsMounted() or E.private.KlixUI.professions.fishing.FromMount) and
	E.private.KlixUI.professions.fishing.EasyCast and FL:IsFishingReady(E.private.KlixUI.professions.fishing.IgnorePole)) then
		return true
	end
end
...
function Pr:GetUpdateLure()
	if E.private.KlixUI.professions.fishing.UseLures then
		...
		if (FL:IsFishingPole()) then
			...
			if ( Pr.LastLure and Pr.LastLure.time and ((Pr.LastLure.time - T.GetTime()) > 0) ) then
				KUI:Print(T.string_format(L["KUI_PROF_RELURE_ERROR"], Pr.LastLure.time - T.GetTime()))
				return false
			end
			...
			if (skill > 0) then
				...
				if ( DoLure and DoLure.id ) then
					...
					if (startTime == 0) then
						...
						Pr.LastLure.time = T.GetTime() + E.private.KlixUI.professions.fishing.relureThreshold
						local id = DoLure.id
						local name = DoLure.n
						return true, id, name;
...
function Pr:FishingInitialize()
	...
	Pr.FishingKey = E.private.KlixUI.professions.fishing.CastButton
	Pr.AddingLure = false
	Pr.LastLure = nil
	Pr.LureState = 0
	Pr.LastCastTime = nil
	Pr.FishingUpdateFrame = T.CreateFrame("Frame", "SLE_FishingUpdateFrame", E.UIParent)
	Pr.FishingUpdateFrame:SetScript("OnUpdate", function(self)
		local stop = true;
		if ( not T.InCombatLockdown() ) then
			FL:ResetOverride();
			if ( Pr.AddingLure ) then
				local sp, sub, txt, tex, st, et, trade, int = T.UnitChannelInfo("player")
