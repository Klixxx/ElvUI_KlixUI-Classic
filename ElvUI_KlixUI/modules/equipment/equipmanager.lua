...
EM.TagsTable = {
	...
	["talent"] = function(tier, column)
		...
		if tier < 0 or tier > 7 then KUI:ErrorPrint(T.string_format(L["KUI_EM_TAG_INVALID_TALENT_TIER"], tier)) return false end
		if column < 0 or column > 3 then KUI:ErrorPrint(T.string_format(L["KUI_EM_TAG_INVALID_TALENT_COLUMN"], column)) return false end
...
function EM:ConditionTable(option)
	...
	while option:match(pattern) do
		...
		T.table_insert(Conditions.options, condition)
	end
	Conditions.set = option:gsub("^%s*", "")
	T.table_insert(EM.SetData, Conditions)
end

function EM:TagsProcess(msg)
	if msg == "" then return end
	T.table_wipe(EM.SetData)
			...
			for index = 1, #Conditions.options do
				...
				for j = 1, #cnd_table do
					...
					if cnd then
						...
						local argTable = {}
						if argument and T.string_find(argument, "%.") then
							...
							if argument and ("/"):split(argument) then
								...
								while argument and ("/"):split(argument) do
									...
									T.table_insert(argTable, put)
								end
							else
								T.table_insert(argTable, argument)
							end
							
							local tag = command:match("^%s*(.+)%s*$")
							if EM.TagsTable[tag] then
								T.table_insert(parsed_cmds, { cmd = command:match("^%s*(.+)%s*$"), arg = argTable })
							else
								KUI:ErrorPrint(T.string_format(L["KUI_EM_TAG_INVALID"], tag))
								T.table_wipe(EM.SetData)
								return
							end
						end
		...
		for _, option in T.ipairs(tagInfo.options) do
			...
			for conditionIndex,conditionInfo in T.ipairs(option.cmds) do
				...
				if not EM.TagsTable[func] then
					KUI:ErrorPrint(T.string_format(L["KUI_EM_TAG_INVALID"], func))
					return nil
				end
...
local function Equip(event)
	...
	for index = 1, T.C_EquipmentSet_GetNumEquipmentSets() do
		local name, _, _, isEquipped = T.C_EquipmentSet_GetEquipmentSetInfo(equipmentSetIDs[index]);
		if isEquipped then
			equippedSet = name
			break
		end
	...
	-- T.print("trueSet:", trueSet)
	if trueSet then
		local SetID = T.C_EquipmentSet_GetEquipmentSetID(trueSet)
		if SetID then
			if not equippedSet or (equippedSet and trueSet ~= equippedSet) then
				T.C_EquipmentSet_UseEquipmentSet(SetID)
			end
		else
			KUI:ErrorPrint(T.string_format(L["KUI_EM_SET_NOT_EXIST"], trueSet))
		end
	end
end
...
function EM:CreateLock()
	...
	local button = T.CreateFrame("Button", "KUI_Equip_Lock_Button", _G["PaperDollFrame"], "UIPanelButtonTemplate")
	button:Size(20, 20)
