...
local EVB = KUI:NewModule("EnhancedVehicleBar")
...
function EVB:CreateExtraButtonSet()
	...
	for i = 1, EVB.NumButtons do
		...
		bar.buttons[i] = LAB:CreateButton(i, T.string_format(bar:GetName().."Button%d", i), bar, nil);
		...
		if E.db.KlixUI.actionbars.transparent then
		...
		-- Style the button backdrop
		bar.buttons[i].backdrop:Styling()
		
		bar.buttons[i]:SetCheckedTexture("")
		T.RegisterStateDriver(bar.buttons[i], 'visibility', '[petbattle] hide; [vehicleui][overridebar][shapeshift][possessbar] show; hide')
	end
end
...
function EVB:BarSize()
	...
	self.size = E.db.KlixUI.actionbars.vehicle.buttonsize
	self.spacing = E.db.KlixUI.actionbars.vehicle.buttonspacing
...
function EVB:BarBackdrop()
	if not self.bar then return end
	self.bar:SetTemplate(E.db.KlixUI.actionbars.vehicle.template)
end

function EVB:Initialize()
	if not E.db.KlixUI.actionbars.vehicle.enable or not E.private.actionbar.enable then return end;
	EVB.NumButtons = E.db.KlixUI.actionbars.vehicle.numButtons

	local visibility = "[petbattle] hide; [vehicleui][overridebar][shapeshift][possessbar] hide;"
	local page = T.string_format("[overridebar] %d; [vehicleui] %d; [possessbar] %d; [shapeshift] 13;", T.GetOverrideBarIndex(), T.GetVehicleBarIndex(), T.GetVehicleBarIndex());
	...
	hooksecurefunc(AB, "PositionAndSizeBar", function(self, barName)
		...
		if (self.db[barName].enabled) and (barName == "bar1") then
			T.UnregisterStateDriver(bar, 'visibility');
			T.RegisterStateDriver(bar, 'visibility', visibility..self.db[barName].visibility);
		end
	end);

	local bar = T.CreateFrame("Frame", "KuiEnhancedVehicleBar", UIParent, "SecureHandlerStateTemplate");
	bar.id = 1
	self.bar = bar;
	...
	T.RegisterStateDriver(bar, 'visibility', '[petbattle] hide; [vehicleui][overridebar][shapeshift][possessbar] show; hide');
	T.RegisterStateDriver(bar, 'page', page);
	...
	self:ButtonsSize()
	E:CreateMover(bar, "KUI_EnhancedVehicleBarMover", L["Enhanced Vehicle Bar"], nil, nil, nil, 'ALL,ACTIONBARS,KLIXUI', nil, "KlixUI,modules,actionbars,vehicle")
...
KUI:RegisterModule(EVB:GetName())